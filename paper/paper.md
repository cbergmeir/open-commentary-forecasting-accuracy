...
<!-- TODO: Start by discussing the M4, its design choices and metrics etc.-->

<!-- Then talk about the ETT benchmark suite from the deep-learning community, how it was very narrow, but still claimed "forecasting", how it fetishised long-horizon forecasting. -->
<!-- Maybe also say that the umbrella "time series forecasting" may often be too broad. -->

<!-- then talk about GIFT-Eval and fev-bench -->


<!-- TODO: Also say somewhere that ARIMA and ETS have single seasonalities and are not designed for high-frequent data. They also predict the conditional mean. So having them lose in comparisons with lots of high-frequent data doesn't say much about their performance on the tasks they are actually good at: lower frequent data and RMSSE evaluation. -->
...
Skill score: is relative to a benchmark, clipped to avoid too much influence from single tasks, and uses geometric mean.

Though the tasks seem a bit better motivated in fev-bench, they are still a value judgement, with e.g. the entire M5 being 3 tasks, and other tasks having single time series.
-->
