...
Let us ask a question: "Who is the world's best athlete?" Though the question is simple, the answer is highly subjective. Which categories should define "best," and how much weight should each receive? Is Michael Jordan "better" than Lionel Messi because he dominated one sport and also played professionally in another? Is Michael Phelps better? The question remains open-ended because everyone brings a different set of criteria and priorities.

To make it more concrete, consider the decathlon. It combines ten disciplines, such as sprinting, long-distance running, shot put, and javelin. Athletes score points in each event over two days, and the highest total wins. Even here, value judgements are unavoidable. While each discipline has a clear local criterion (for example, farther javelin throws are better), aggregating across disciplines requires a mapping to a common scale. The decathlon resolves this through a point system that encodes explicit design choices about baselines, curvature, and aggregation.
...
This is why rankings appear so quickly in both academic studies and practitioner reports: they provide a single summary of performance. Yet that summary is never neutral. Every ranking reflects a set of evaluation choices about what is measured, how it is scaled, and how results are aggregated.
...
<!-- TODO: extend teh last sentence with some more examples from the ML literature. -->
