...
Let us ask you a question: "Who is the world's best athlete?" Though the question is simple, the answer turns out to be highly subjective. What are the categories to measure "best", which importance to give to each of them? Is Michael Jordan "better" than Lionel Messi because he dominated one sport and also played professionally in another one? Is Michael Phelbs maybe better? The question is very subjective and open-ended because every body will have their own set of criteria and decisions of what to give importance to. So let's make the question slightly more tangible. We could argue that we want to have the most well-rounded athlete as the best athlete, and that we decide to stick to decathlon, which has 10 different disciplines of athletics, such as springing, long-distance running, shot-put, javelin, and others. Athletes get points in each of these competitions, held over two days, and in the end whoever scores the most points wins. But again, there are quite subjective decisions involved. While it may be easy to judge each discipline separately (the further you throw the javelin, the better), when it comes to aggregate over the disciplines, already value judgement is involved. Performances from different disciplines need to be transformed to a common scale and aggregated. The decathlon solves this with a point system that embodies explicit design choices about baselines, curvature, and aggregation. 

Now let me ask you another question: Which is the best forecasting method? Forecast evaluation works in exactly the same way. Once we compare methods across horizons, series, datasets, or business units, the winner depends on the scoring system we have chosen, not only on the intrinsic quality of the model.

Both in research and in real-world industry settings, people often ask a simple question: what is the best forecasting method? In academic papers, benchmark studies, and practitioner talks, that question is usually followed immediately by a ranking table. But rankings in forecasting are never purely about forecasting methods. They are also about the evaluation system used to compare them.
...
If forecasts need to add up, e.g., in a hierarchical setting, they also need to be conditional means. If the data is intermittent (over 50% of the data is zero), the median will be a zero, so that under MAE beating a constant zero forecast is oftentimes not possible in this setting.
